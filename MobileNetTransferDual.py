...
# 분류기의 신뢰도 이 값 이상이면 사용자 결과 우선 선택
...
def build_dual_inference_model(custom_model):
    ...
    # 내부 레이어 이름 충돌을 피하기 위해 외부 이름 변경
    imagenet_model._name = 'mobilenet_imagenet_full'
    inputs = tf.keras.Input(shape=IMG_SIZE + (3,), name='input_image')

    imagenet_pred = imagenet_model(inputs, training=False)
    # ImageNet 1000클래스 softmax로 출력 해서 사용
    imagenet_pred = layers.Activation('linear', name='imagenet_pred')(imagenet_pred)

    custom_pred = custom_model(inputs, training=False)
    custom_pred = layers.Activation("linear", name="custom_pred")(custom_pred)

    dual_model = models.Model(
        inputs,
        [imagenet_pred, custom_pred],
        name='dual_head_inference',
    )

    return dual_model


def predict_image(dual_model, img_path, class_names, top_k=3, verbose=True):
    # 이미지 로드 + MobileNet 전처리
    ...
    # 듀얼 추론(인식)
    imagenet_pred, custom_pred = dual_model.predict(arr, verbose=0)
    ...
    if custom_pred.shape[-1] == 1:
        ...
        if p >= 0.5:
            custom_label, custom_conf = class_names[1], p
        else:
            custom_label, custom_conf = class_names[0], 1.0 - p
    else:
        #다중 분류
        idx = int(np.argmax(custom_pred[0]))
        custom_label = class_names[idx]
        costom_conf = float(custom_pred[0,idx])

    #최종 판단하는 부분
    if costom_conf >= CUSTOM_CONFIDENCE_THRESHOLD:
        verdict = f"[사용자 우선 {custom_label} ({costom_conf *100:.1f}%)"
    else:
        top1 = imagenet_top[0]
        verdict = (
            f"[ImageNet우선] {top1[1]} ({top1[2]*100:.1f}%)"
            f"(사용자 후보 : {custom_label} {costom_conf*100:.1f}%)"
        )

    if verbose:
        print(f"\n --- 예측 : {img_path} ----")
        print(" [ImageNet Top-K")
        for _, name, score in imagenet_top:
            print(f"   - {name:25s} {score*100:6.2f}%")

        print(f"   [사용자 분류기] {custom_label} {costom_conf*100:6.2f}%")
        print(f"   >> {verdict}")

    return {
        "imagenet": imagenet_top,
        "custom": (custom_label, costom_conf),
        "verdict": verdict
    }
...
def main():
    ...
    #학습용 모델
    custom_model, base_model, loss_fn = build_custom_model(num_classes)
    print("\n[1단계] 사용자 분류기 헤드 학습")
    ...
    print(f'\n모델 저장 완료 {CUSTOM_MODEL_PATH}')
    print(f'클래스 이름 저장 완료: {CLASS_NAMES_PATH}')
    # 1단계+2단계 학습 곡선을 하나의 이미지로 그려서 저장
    plot_history(history_head, history_fine)

    val_loss, val_acc = custom_model.evaluate(val_ds)
    print(f'\n 최종 검증 정확도: {val_acc:.4f} / 검증 손실: {val_loss:.4f}')

    # 듀얼 헤드 추론
    dual_model = build_dual_inference_model(custom_model)
    dual_model.summary()
    dual_model.save(DUAL_MODEL_PATH)
    print(f"듀얼 헤드 추론 모델 저장 완료{DUAL_MODEL_PATH}")

    #검증 폴더에서 몇장의 샘플 추론
    sample_dir = VAL_DIR if os.path.isdir(VAL_DIR) else DATASET_DIR
    print(f"\n[샘플 추론 {sample_dir}에서 클래스 별 1장씩 테스트")
    for cls in class_names:
        ...
        predict_image(dual_model, os.path.join(cls_dir, files[0]), class_names)
